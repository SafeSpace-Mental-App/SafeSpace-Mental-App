...
export default function Onboarding() {
  ...
  return (
    <div className="onb-root" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      {/* Skip (hidden on last screen) */}
      {!last && (
        <button className="onb-skip" onClick={skip} aria-label="Skip onboarding">
          Skip
        </button>
      )}
      ...
      <div className="onb-layout">
        ...
        <div className="onb-text">
          ...
          {/* Progress bar */}
          <div className="onb-progress" aria-label="progress" role="progressbar" aria-valuenow={Math.round(progress*100)} aria-valuemin={0} aria-valuemax={100}>
          ...
          <div className="onb-cta">
            {!last ? (
              ...
              <Link to="/register" className="onb-primary onb-link-btn">
                Get Started
              </Link>
            )}
          </div>
      ...
      {index > 0 && (
        <button className="onb-nav onb-left" onClick={gotoPrev} aria-label="Previous">
          {/* left chevron svg */}
          <svg width="22" height="22" viewBox="0 0 24 24" aria-hidden="true"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/></svg>
        </button>
      )}
      {!last && (
        <button className="onb-nav onb-right" onClick={gotoNext} aria-label="Next">
          {/* right chevron svg */}
          <svg width="22" height="22" viewBox="0 0 24 24" aria-hidden="true"><path d="M8.59 16.59 13.17 12 8.59 7.41 10 6l6 6-6 6z"/></svg>
        </button>
      )}
      ...
      <div className="onb-dots" aria-hidden="true">
        {slides.map((_, i) => (
          <span key={i} className={`onb-dot ${i <= index ? "is-active" : ""}`} />
        ))}
      </div>
    </div>
  );
